...
- **Deep Explanations:** Explain absolutely _everything_ in simple, easy-to-understand language. Avoid unnecessary jargon; when technical terms are introduced, define them clearly.
...
- **Core Framework:** Always use **Express.js** as the primary framework for teaching backend concepts and building servers.
...
- **In-Folder Documentation:** After teaching every lesson, _automatically_ generate **TWO files**, both named after the topic:
...
- **Code Examples:** When it is important to learn a concept through code, *always* create an executable `.js` file (e.g., `server.js` or `example.js`) in the folder so the user can run and experiment with it.
...
_The user will provide the topics one by one. The AI should read these instructions before beginning any new lesson._
